    ...
    <el-dialog
      ...
      <el-form v-model="selectedRole" style="text-align: left" ref="dataForm">
        <el-form-item label="角色名" label-width="120px" prop="username">
          <el-input v-model="selectedRole.roleName" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="角色描述" label-width="120px" prop="name">
          <el-input v-model="selectedRole.roleDesc" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="功能配置" label-width="120px" prop="perms">
          <el-checkbox-group v-model="selectedPermsIds">
            <el-checkbox v-for="(perm,i) in perms" :key="i" :label="perm.id">{{perm.rightDesc}}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
...
export default {
  ...
  methods: {
      ...
      if (role.id !== 1) {
        this.$confirm('是否更改角色状态？', '提示', {
          ...
          this.$axios.put('/admin/role/status', {
            ...
            if (resp && resp.data.code === 200) {
              if (value) {
                this.$message('角色 [' + role.roleDesc + '] 已启用')
              } else {
                this.$message('角色 [' + role.roleDesc + '] 已禁用')
              }
            }
          })
        }).catch(() => {
    ...
    deleteRole (role) {
      this.$axios.delete('/admin/role?rid=' + role.id).then(resp => {
        if (resp && resp.data.code === 200) {
